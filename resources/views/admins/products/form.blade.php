    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const openImagePicker = (editor) => {
                const files = window.mediaImages || [];
                if (!files.length) {
                    alert('Chưa có ảnh trong thư viện @img');
                    return;
                }
                ...
                modal.innerHTML = `
                    <div style="background:#fff;padding:20px;border-radius:12px;max-height:80vh;overflow:auto;width:520px;">
                        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">
                            <h4 style="margin:0;">Chọn ảnh từ @img</h4>
                            <button type="button" data-close style="border:none;background:none;font-size:20px;cursor:pointer;">&times;</button>
                        </div>
                        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-top:10px;">
                            ${files.map(file => `
                                <button type="button" data-url="${file.url}" data-name="${file.name}" style="border:1px solid #e2e8f0;background:#fff;padding:0;border-radius:12px;cursor:pointer;overflow:hidden;">
                                    <img src="${file.url}" alt="${file.name}" style="width:100%;height:120px;object-fit:cover;display:block;">
                ...
                const handleClick = (event) => {
                    ...
                    if (btn) {
                        ...
                        editor.insertContent(`<img src="${btn.dataset.url}" alt="${btn.dataset.name}">`);
                        markDirty();
                        closeModal();
                    }
                };
