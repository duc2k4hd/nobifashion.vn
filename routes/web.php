...
use App\Http\Controllers\Admins\ToolsController;
use App\Http\Controllers\Admins\TrashController;
use App\Http\Controllers\Admins\VoucherAnalyticsController;
use App\Http\Controllers\Admins\YodyCrawlerController;
...
Route::get('/order/track', [OrderTrackingController::class, 'show'])->name('client.order.track');
Route::post('/order/track', [OrderTrackingController::class, 'lookup'])->name('client.order.track.lookup');
Route::get('/order/{id}', [OrderController::class, 'show'])->name('client.order.show');
...
Route::post('/api/contact', [ContactController::class, 'store'])->name('client.contact.store');
...
// Chat CSKH trực tiếp (YDC Index API & Database Storage)
Route::prefix('chat')->name('client.chat.')->group(function () {
    Route::get('/init', [ChatBotController::class, 'getInitData'])->name('init');
    Route::get('/messages', [ChatBotController::class, 'getMessages'])->name('messages');
    Route::post('/send', [ChatBotController::class, 'sendMessage'])->name('send')->middleware('throttle:40,1');
...
// Sitemap routes
Route::get('/sitemap', [SitemapController::class, 'html'])->name('client.sitemap.html');
Route::get('/sitemap.xml', [SitemapController::class, 'index'])->name('sitemap.index');
Route::get('/sitemap-index.xml', [SitemapController::class, 'index'])->name('sitemap.index.alias');
Route::get('/sitemap-posts.xml', [SitemapController::class, 'posts'])->name('sitemap.posts');
Route::get('/sitemap-posts-{page}.xml', [SitemapController::class, 'posts'])->name('sitemap.posts.page');
...
Route::prefix('admin')->name('admin.')->group(function () {
    ...
    Route::middleware(['auth:web', 'admin'])->group(function () {
        ...
        Route::prefix('newsletters')->name('newsletters.')->group(function () {
            ...
            Route::get('/campaign', [AdminNewsletterController::class, 'showCampaignForm'])->name('campaign');
            Route::get('/{id}', [AdminNewsletterController::class, 'show'])->name('show');
            Route::delete('/{id}', [AdminNewsletterController::class, 'destroy'])->name('destroy');
            Route::post('/{id}/change-status', [AdminNewsletterController::class, 'changeStatus'])->name('change-status');
            Route::post('/{id}/resend-verify', [AdminNewsletterController::class, 'resendVerifyEmail'])->name('resend-verify');
            Route::post('/send', [AdminNewsletterController::class, 'sendBulkEmail'])->name('send');
        });
        ...
        Route::prefix('media')->name('media.')->group(function () {
            ...
            Route::post('/library', [MediaLibraryController::class, 'store'])->name('library.store');
            Route::put('/library/{id}', [MediaLibraryController::class, 'update'])->name('library.update');
            Route::delete('/library/bulk-delete', [MediaLibraryController::class, 'bulkDelete'])->name('library.bulk-delete');
            Route::delete('/library/{id}', [MediaLibraryController::class, 'destroy'])->name('library.destroy');
        });
        ...
        Route::prefix('vouchers')->name('vouchers.')->group(function () {
            ...
            Route::get('/analytics', [VoucherAnalyticsController::class, 'dashboard'])->name('analytics');
            Route::get('/analytics/{voucherId}', [VoucherAnalyticsController::class, 'voucherDetail'])->name('analytics.detail');
            Route::get('/analytics/api/revenue-trend/{voucherId}', [VoucherAnalyticsController::class, 'getRevenueTrendData'])->name('analytics.api.revenue-trend');
            Route::get('/analytics/api/overall-stats', [VoucherAnalyticsController::class, 'getOverallStats'])->name('analytics.api.overall-stats');
        ...
        Route::prefix('cart-items')->name('cart-items.')->group(function () {
            ...
            Route::get('/{cartItem}/edit', [CartItemController::class, 'edit'])->name('edit');
        ...
        Route::prefix('trash')->name('trash.')->group(function () {
            ...
            Route::patch('/{type}/{id}', [TrashController::class, 'restore'])->name('restore');
        ...
        Route::prefix('comments')->name('comments.')->group(function () {
            ...
            Route::post('/{comment}/toggle-approve', [CommentController::class, 'toggleApprove'])->name('toggle-approve');
        ...
        Route::prefix('canifa-crawler')->name('canifa-crawler.')->group(function () {
            Route::get('/', [CanifaCrawlerController::class, 'index'])->name('index');
            Route::post('/crawl', [CanifaCrawlerController::class, 'crawl'])->name('crawl');
            Route::post('/generate-urls', [CanifaCrawlerController::class, 'generateUrls'])->name('generate-urls');
        });

        // Routine Crawler Tool
        Route::prefix('routine-crawler')->name('routine-crawler.')->group(function () {
            Route::get('/', [RoutineCrawlerController::class, 'index'])->name('index');
            Route::post('/crawl', [RoutineCrawlerController::class, 'crawl'])->name('crawl');
        });

        // Onoff Crawler Tool
        Route::prefix('onoff-crawler')->name('onoff-crawler.')->group(function () {
            Route::get('/', [OnoffCrawlerController::class, 'index'])->name('index');
            Route::post('/crawl', [OnoffCrawlerController::class, 'crawl'])->name('crawl');
        });

        // Coolmate Crawler Tool
        Route::prefix('coolmate-crawler')->name('coolmate-crawler.')->group(function () {
            Route::get('/', [CoolmateCrawlerController::class, 'index'])->name('index');
            Route::post('/crawl', [CoolmateCrawlerController::class, 'crawl'])->name('crawl');
            Route::get('/download/{filename}', [CoolmateCrawlerController::class, 'download'])
        ...
        Route::prefix('yody-crawler')->name('yody-crawler.')->group(function () {
            Route::get('/', [YodyCrawlerController::class, 'index'])->name('index');
            Route::post('/crawl', [YodyCrawlerController::class, 'crawl'])->name('crawl');
            Route::post('/delete-product/{sku}', [YodyCrawlerController::class, 'deleteProduct'])
                ->name('delete-product');
            Route::post('/clear-temp', [YodyCrawlerController::class, 'clearTemp'])
                ->name('clear-temp');
            Route::get('/download/{filename}', [YodyCrawlerController::class, 'download'])
                ->where('filename', '.*')
                ->name('download');
            Route::get('/preview-image/{path}', [YodyCrawlerController::class, 'previewImage'])
                ->where('path', '.*')
                ->name('preview-image');
        });
    });
